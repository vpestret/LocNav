...
for midx in range(NUM_MEAN):
    ...
    #signal = make_phase_noise(PN_SC_MIN, PN_SC_MAX, PHASE_DEV * 2)
    #acc_sig2 += np.abs(np.fft.fft(signal)) ** 2
# normalize
acc_sig1 /= np.max(acc_sig1)
#acc_sig2 /= np.max(acc_sig2)
#ref = np.abs(np.fft.fft(np.cos(2 * np.pi * 0.5 * xvals))) ** 2
#ref /= (np.max(ref) * 100)
#ref[0] = 1 # compensate absense of mean

spectre1 = 10 * np.log(acc_sig1) / np.log(10)
#spectre2 = 10 * np.log(acc_sig2) / np.log(10)
...
#plt.plot(xvals[D_L:D_H] * SAMP_FREQ, spectre2[D_L:D_H], 'b-')
